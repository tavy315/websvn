...
// removeAccents
//
// Remove all the accents from a string. This function doesn't seem ideal,
// but expecting everyone to install 'unac' seems a little excessive as well...
...
if ($rep) {
    ...
    if (!empty($history)) {
        ...
        foreach ($entries as $revision) {
            ...
            if ($match) {
                ...
                if (empty($precisePath)) {
                    ...
                    if ($isDir && $precisePath{strlen($precisePath) - 1} != '/') {
                        $precisePath .= '/';
                    }
                }
        ...
        if ($dosearch && !$found) {
            ...
            if ($dosearch && $numSearchResults > 0) {
                $vars['logsearch_nomorematches'] = true;
            }
        }
