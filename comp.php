...
if ($rep) {
    ...
    $vars['reverselink'] = '<a href="' . $url . 'compare%5B%5D=' . urlencode($path2) . '@' . $rev2 . '&amp;compare%5B%5D=' . urlencode($path1) . '@' . $rev1 . '&amp;manualorder=1' . ($ignoreWhitespace ? '&amp;ignorews=1'
            : '') . '">' . $lang['REVCOMP'] . '</a>';
    if (!$ignoreWhitespace) {
        $vars['ignorewhitespacelink'] = '<a href="' . $url . 'compare%5B%5D=' . urlencode($path1) . '@' . $rev1 . '&amp;compare%5B%5D=' . urlencode($path2) . '@' . $rev2 . ($manualOrder ? '&amp;manualorder=1'
                : '') . '&amp;ignorews=1">' . $lang['IGNOREWHITESPACE'] . '</a>';
    } else {
        $vars['regardwhitespacelink'] = '<a href="' . $url . 'compare%5B%5D=' . urlencode($path1) . '@' . $rev1 . '&amp;compare%5B%5D=' . urlencode($path2) . '@' . $rev2 . ($manualOrder ? '&amp;manualorder=1'
                : '') . '">' . $lang['REGARDWHITESPACE'] . '</a>';
    }
    ...
    function clearVars()
    {
        ...
        if ($ignoreWhitespace && $index > 1) {
            $endBlock = false;
            $previous = $index - 1;
            if ($listing[$previous]['endpath']) {
                $endBlock = 'newpath';
            } else {
                if ($listing[$previous]['enddifflines']) {
                    $endBlock = 'difflines';
                }
            }
            if ($endBlock !== false) {
            ...
            while (!feof($diff)) {
                ...
                if ($indiff) {
                        ...
                        if (strlen($line) > 0 && ($line[0] == ' ' || $line[0] == '+' || $line[0] == '-')) {
                            ...
                            switch ($line[0]) {
                                case ' ':
                                    ...
                                    if ($debug) {
                                        print 'Including as diff: ' . $subline . '<br />';
                                    }
                                    break;
                                ...
                                case '+':
                                    ...
                                    if ($debug) {
                                        print 'Including as added: ' . $subline . '<br />';
                                    }
                                    break;
                                ...
                                case '-':
                                    ...
                                    if ($debug) {
                                        print 'Including as removed: ' . $subline . '<br />';
                                    }
                                    break;
                            }
                            $index++;
                    ...
                    if (!strncmp($line, '@@', 2)) {
                        ...
                        if ($debug) {
                            print 'sline = "' . $sline . '", eline = "' . $eline . '"<br />';
                        }
                        // Check that this isn't a file deletion
                        if ($sline == 0 && $eline == 0) {
                            $line = fgets($diff);
                            if ($debug) {
                                print 'Ignoring: "' . $line . '"<br />';
                            }
                            ...
                            $getLine = false;
                            if ($debug) {
                                print 'Unignoring previous - marking as deleted<br />';
                            }
                ...
                if (strncmp(trim($line), 'Index: ', 7) == 0) {
                    ...
                    if ($debug) {
                        echo 'Creating node ' . $node . '<br />';
                    }

                    // Skip past the line of ='s
                    $line = fgets($diff);
                    if ($debug) {
                        print 'Skipping: ' . $line . '<br />';
                    }

                    // Check for a file addition
                    $line = fgets($diff);
                    if ($debug) {
                        print 'Examining: ' . $line . '<br />';
                    }
                ...
                if (strncmp(trim($line), 'Property changes on: ', 21) == 0) {
                    ...
                    if ($debug) {
                        print 'Properties on ' . $propnode . ' (cur node $ ' . $node . ')';
                    }
                    if ($propnode != $node) {
                        if ($node) {
                            $listing[$index++]['endpath'] = true;
