...
if ($rep) {
    ...
    if ($history && isset($zipped) && in_array($extension, $zipped) && $rep->hasReadAccess($path, false)) {
        $base = basename($path);
    ...
    if (!$rep->getIgnoreWebSVNContentTypes()) {
        $setupContentType = @$contentType[$extension];
    }
    ...
    } elseif (!empty($setupContentType)) {
        $mimeType = $setupContentType;
    } elseif (!empty($svnMimeType)) {
    ...
    } elseif (!$svnrep->isFile($path, $rev, $peg)) {
        $vars['error'] = $lang['NOPATH'];
