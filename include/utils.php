...
function create_anchors($text)
{
    ...
    $ret = preg_replace('#\b(?<!href=")([a-z]+?)://(\S*)([\w/]+)#i', '<a href="\\1://\\2\\3" target="_blank">\\1://\\2\\3</a>', $ret);

    // Now match anything beginning with www, as long as it's not //www since they were matched above
    $ret = preg_replace('#\b(?<!//)www\.(\S*)([\w/]+)#i', '<a href="http://www.\\1\\2" target="_blank">www.\\1\\2</a>', $ret);

    // Match email addresses
    $ret = preg_replace('#\b([\w\-_.]+)@([\w\-.]+)\.(\w{2,})\b#i', '<a href="mailto:\\1@\\2.\\3">\\1@\\2.\\3</a>', $ret);
...
{
    global $config;
    if (!$config->useAcceptedLanguages()) {
        return $default;
    }
