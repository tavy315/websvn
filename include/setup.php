...
$config->checkIfWindowsServer();

// Set up locwebsvnhttp
// Note: we will use nothing in MultiViews mode so that the URLs use the root directory by default.
...
// List of extensions recognized by enscript.
$extEnscript = array(
    '.ada'           => 'ada',
    '.adb'           => 'ada',
    '.ads'           => 'ada',
...
if (!empty($_REQUEST['language'])) {
    ...
    setcookie('storedlang', $language, time() + 307584000, '/'); // time() + (60 * 60 * 24 * 356 * 10)
    setcookie('storedsesslang', $language);
} elseif (isset($_COOKIE['storedlang'])) {
    $language = $_COOKIE['storedlang'];
} elseif (isset($_COOKIE['storedsesslang'])) {
...
if (!empty($_REQUEST['template'])) {
    ...
    setcookie('storedtemplate', $template, time() + 307584000, '/'); // time() + (60 * 60 * 24 * 365 * 10)
    setcookie('storedsesstemplate', $template);
} elseif (isset($_COOKIE['storedtemplate'])) {
    $template = $_COOKIE['storedtemplate'];
} elseif (isset($_COOKIE['storedsesstemplate'])) {
